...
#            if os.getcwd().startswith('/dls'):
            if os.path.isdir('/dls'):
                if self.external_software['qsub_array']:
                    ...
                    self.Logfile.insert('         qsub -P labxchem -q shared -t 1:{0!s} -tc {1!s} acedrg_master.sh'.format(str(counter), self.max_queue_jobs))
                    # self.Logfile.insert('         qsub -P labxchem -q medium.q -t 1:{0!s} -tc {1!s} acedrg_master.sh'.format(str(counter), self.max_queue_jobs))
                    os.system('qsub -P labxchem -q shared -t 1:{0!s} -tc {1!s} -N acedrg acedrg_master.sh'.format(str(counter), self.max_queue_jobs))
                ...
                for i in range(counter):
                    self.Logfile.insert('qsub -q shared -N acedrg xce_acedrg_{0!s}.sh'.format((str(i+1))))
    ...
    def run_script(self):
        ...
        if os.path.isdir('/dls'):
            if self.external_software['qsub_array']:
                ...
                self.Logfile.insert('using the following command:')
                self.Logfile.insert(
                    'qsub -P labxchem -q shared -t 1:{0!s} -tc {1!s} autofit_ligand_master.sh'.format(str(self.n),
                    # 'qsub -P labxchem -q medium.q -t 1:{0!s} -tc {1!s} autofit_ligand_master.sh'.format(str(self.n),
                                                                                               self.max_queue_jobs))
                os.system(
                    'qsub -P labxchem -q shared -t 1:{0!s} -tc {1!s} autofit_ligand_master.sh'.format(str(self.n - 1),
            ...
            for i in range(1, self.n):
                self.Logfile.insert('qsub -q shared xce_autofit_ligand_{0!s}.sh'.format(str(i)))
                # self.Logfile.insert('qsub -q medium.q xce_autofit_ligand_{0!s}.sh'.format(str(i)))
                os.system('qsub -q shared xce_autofit_ligand_{0!s}.sh'.format(str(i)))
...
#                    'cd %s\n' %os.path.join(self.initial_model_directory,xtal,'dimple',visit_run_autoproc,'dimple') +
        ...
        if os.path.isdir('/dls'):
            if self.external_software['qsub_array']:
                ...
                self.Logfile.insert('submitting array job with maximal 100 jobs running on cluster')
                self.Logfile.insert('using the following command:')
                self.Logfile.insert('qsub -P labxchem -q shared -t 1:{0!s} -tc {1!s} dimple_master.sh'.format(str(n+1), self.max_queue_jobs))
                ...
                if self.using_remote_qsub_submission:
                    os.system(str(self.remote_qsub_submission).replace("qsub'", str('cd ' + str(os.getcwd()) + '; ' + 'qsub -P labxchem -q shared -t 1:{0!s} -tc {1!s} dimple_master.sh'
            ...
            self.Logfile.insert('WARNING: this could potentially lead to a crash...')
            for i in range(n+1):
                self.Logfile.insert('qsub -q shared -N dimple xce_dimple_{0!s}.sh'.format((str(i+1))))
    ...
    def run_script(self):
        ...
        if os.path.isdir('/dls'):
            if self.external_software['qsub_array']:
                ...
                self.Logfile.insert('submitting array job with maximal 100 jobs running on cluster')
                self.Logfile.insert('using the following command:')
                self.Logfile.insert('qsub -P labxchem -q shared -t 1:{0!s} -tc {1!s} {2!s}_master.sh'.format(str(self.n), self.max_queue_jobs, self.pipeline))
                ...
                os.system('qsub -P labxchem -q shared -t 1:{0!s} -tc {1!s} {2!s}_master.sh'.format(str(self.n-1), self.max_queue_jobs, self.pipeline))
            ...
            self.Logfile.insert('WARNING: this could potentially lead to a crash...')
            for i in range(1,self.n):
                self.Logfile.insert('qsub -q shared xce_{0!s}_{1!s}.sh'.format(self.pipeline,str(i)))
                # self.Logfile.insert('qsub -q medium.q xce_{0!s}_{1!s}.sh'.format(self.pipeline,str(i)))
                os.system('qsub -q shared xce_{0!s}_{1!s}.sh'.format(self.pipeline,str(i)))
