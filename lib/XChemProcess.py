...
#                    script+='$CCP4/bin/xia2 pipeline='+pipeline+' '+ref_option+' '+spg_option+' '+reso_limit_option+' '+cc_half_option+' '+image_dir+'\n'
        ...
        if os.getcwd().startswith('/dls'):
            if self.external_software['qsub_array']:
                ...
                os.system('qsub -P labxchem -q shared -N xia2 -t 1:{0!s} -tc {1!s} xia2_master.sh'.format(str(i+1), self.max_queue_jobs))
            ...
            for n in range(i+1):
                ...
                os.system('qsub -q shared -N xia2 xce_xia2_{0!s}.sh'.format((str(n+1))))
